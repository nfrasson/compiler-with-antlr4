...
cmdIF		: 'se' { stack.push(new ArrayList<Command>()); strExpr = ""; currentCommandIf = new CommandIf(); } 
               AP 
               expr
               OPREL { strExpr += _input.LT(-1).getText(); }
               expr 
               ...
               ( 'senao' { stack.push(new ArrayList<Command>()); } comando+ { currentCommandIf.setFalseList(stack.pop()); })? 
               'fimse' { stack.peek().add(currentCommandIf); }
			;
...
cmdEscrita   : 'escreva' AP termo { Command cmdWrite = new CommandWrite(_input.LT(-1).getText()); stack.peek().add(cmdWrite); } FP PV { rightType = null; } ;
...
termo        : ID { 
              ...
              | NUM { if (rightType == null) { rightType = Types.NUMBER; } else if (rightType.getValue() < Types.NUMBER.getValue()) { rightType = Types.NUMBER; } }
			  | TEXTO { if (rightType == null) { rightType = Types.TEXT; } else if (rightType.getValue() < Types.TEXT.getValue()) { rightType = Types.TEXT; } }
			;

exprl        : ( OP { strExpr += _input.LT(-1).getText(); } termo { strExpr += _input.LT(-1).getText(); } )*;
